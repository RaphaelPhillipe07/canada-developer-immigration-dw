...
Saída:
    docs/Trabalho_Escrito_Nota1.docx
"""
...
OUT = os.path.join(ROOT, "docs", "Trabalho_Escrito_Nota1.docx")
...
def picture(rel):
    doc.add_picture(os.path.join(ROOT, "presentation", "images", rel), width=Inches(5.9))
...
center("Raphael Pihllipe da Silva Silverio", size=12, space_after=24)

center("MACEIÓ - AL", bold=True, size=12, space_after=0)
center("Março de 2026", bold=True, size=12, space_after=0)
page_break()
...
p("O projeto abrange as 13 províncias e territórios canadenses e integra três bases oficiais: imigração e habitação do Censo 2021 (Statistics Canada) e salários de Software Developers and Programmers (NOC 21232) do Job Bank/ESDC, com referência salarial 2023–2024.")
...
bullet("Modelar três esquemas estrela com dimensões conformadas e surrogate keys.")
bullet("Documentar o dicionário de dados das bases OLTP e do Data Warehouse.")
bullet("Realizar a análise exploratória das bases de origem.")
bullet("Justificar e analisar o impacto de toda alteração realizada na base OLTP.")
...
table(
    ...
    [
        ...
        ("T — Temporal", "Até a data final da disciplina, conforme cronograma do docente."),
    ],
    widths=[1.6, 4.9],
)

# ---------------------------------------------------------------- 9. DESCRIÇÃO DO SISTEMA / DADOS OLTP
h1("9. Descrição do Sistema e dos Dados (OLTP)")
p("Seguindo a abordagem do exemplo da disciplina, os arquivos públicos em CSV/ZIP são tratados como bases operacionais de origem (OLTP). Os arquivos brutos são preservados sem alteração na pasta raw/ com URL, data de extração e checksum; filtros e padronizações ocorrem apenas em staging e no DW.")
...
# ---------------------------------------------------------------- 11. DICIONÁRIO OLTP
h1("11. Dicionário de Dados da Base OLTP")
...
# ---------------------------------------------------------------- 12. EDA
h1("12. Análise Exploratória da Base OLTP")
...
h1("13. Alterações na Base OLTP e Impacto")
p("Os arquivos brutos são preservados sem alteração. Toda transformação ocorre em staging/DW e é justificada a seguir.")
table(
    ...
    [
        ...
        ("Filtro StatCan imigração: Age/Gender totais e Place of birth em {Brazil, Total – Place of birth}", "tabela completa → 26 registros", "Garante numerador (Brazil) e denominador (Total) comparáveis, sem quebra por idade/gênero."),
        ...
        ("Normalização geográfica", "nome inglês da província como chave de cruzamento", "Job Bank usa prov/ER_Code e StatCan usa DGUID; o nome em inglês é a chave comum. Quebec (inglês) ≠ Québec (francês, não usado)."),
...
p("Impacto geral: o Job Bank cai de 44.376 para 9 registros provinciais efetivamente usados no fato_salario; a habitação cai de 2.988 para 117 registros; a imigração usa 26 registros. Nenhuma linha é excluída dos brutos — apenas filtrada nas camadas seguintes.")
page_break()

# ---------------------------------------------------------------- 14. MODELO DIMENSIONAL
h1("14. Modelo Lógico/Físico do Data Warehouse (Star Schema)")
p("São três esquemas estrela com dimensões conformadas. A separação evita repetir salários em cada período de imigração e impede somas incorretas.")

h2("14.1 Diagrama")
mono("dim_provincia ──┬── fato_salario ── dim_ocupacao")
mono("                 │        └────────── dim_tempo_salario")
mono("                 ├── fato_imigracao ── dim_pais_nascimento")
mono("                 │        ├─────────── dim_periodo_imigracao")
mono("                 │        └─────────── dim_tempo_censo")
mono("                 └── fato_habitacao ── dim_tenencia")
mono("                          ├─────────── dim_indicador_habitacao")
mono("                          └─────────── dim_tempo_censo")
...
dw_tables = {
    "dim_provincia": [
        ("sk_provincia", "NUMBER(10)", "PK", "Surrogate key (SEQ_DIM_PROVINCIA)"),
...
p("O script abaixo cria tablespaces, sequences, tabelas de staging, dimensões e fatos do DW com constraints e índices. Identificadores com até 30 caracteres (compatível com Oracle 11g+).")
...
h1("Referências")
for ref in [
    "Statistics Canada. Table 98-10-0307-01 — Immigrant status and period of immigration by place of birth. Censo 2021. Disponível em: https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=9810030701",
    "Government of Canada / ESDC. Job Bank Wages — Software developers and programmers (NOC 21232). Release 2025. Disponível em: https://open.canada.ca/data/en/dataset/adad580f-76b0-4502-bd05-20c125de9116",
    "Statistics Canada. Table 98-10-0258-01 — Housing indicators by tenure. Censo 2021. Disponível em: https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=9810025801",
]:
    p(ref, size=12)
