...
CREATE SEQUENCE SEQ_DIM_DISP_SALARIO START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE;
...
-- ==============================
-- GATILHOS DE CHAVE SUBSTITUTA
-- ==============================

-- Cada gatilho atribui a surrogate key a partir da sequence correspondente
-- quando a carga nao informa o valor. A condicao WHEN (NEW.<pk> IS NULL)
-- preserva chaves informadas explicitamente, o que permite recarga idempotente.
-- Executar apos a criacao das tabelas.

CREATE OR REPLACE TRIGGER TRG_DIM_PROVINCIA
BEFORE INSERT ON dim_provincia
FOR EACH ROW
WHEN (NEW.sk_provincia IS NULL)
BEGIN
    :NEW.sk_provincia := SEQ_DIM_PROVINCIA.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_DIM_OCUPACAO
BEFORE INSERT ON dim_ocupacao
FOR EACH ROW
WHEN (NEW.sk_ocupacao IS NULL)
BEGIN
    :NEW.sk_ocupacao := SEQ_DIM_OCUPACAO.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_DIM_TEMPO_SALARIO
BEFORE INSERT ON dim_tempo_salario
FOR EACH ROW
WHEN (NEW.sk_tempo_salario IS NULL)
BEGIN
    :NEW.sk_tempo_salario := SEQ_DIM_TEMPO_SALARIO.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_DIM_DISP_SALARIO
BEFORE INSERT ON dim_disponibilidade_salario
FOR EACH ROW
WHEN (NEW.sk_disponibilidade_salario IS NULL)
BEGIN
    :NEW.sk_disponibilidade_salario := SEQ_DIM_DISP_SALARIO.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_DIM_TEMPO_CENSO
BEFORE INSERT ON dim_tempo_censo
FOR EACH ROW
WHEN (NEW.sk_tempo_censo IS NULL)
BEGIN
    :NEW.sk_tempo_censo := SEQ_DIM_TEMPO_CENSO.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_DIM_PAIS_NASCIMENTO
BEFORE INSERT ON dim_pais_nascimento
FOR EACH ROW
WHEN (NEW.sk_pais_nascimento IS NULL)
BEGIN
    :NEW.sk_pais_nascimento := SEQ_DIM_PAIS_NASCIMENTO.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_DIM_PERIODO_IMIGRACAO
BEFORE INSERT ON dim_periodo_imigracao
FOR EACH ROW
WHEN (NEW.sk_periodo_imigracao IS NULL)
BEGIN
    :NEW.sk_periodo_imigracao := SEQ_DIM_PERIODO_IMIGRACAO.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_DIM_TENENCIA
BEFORE INSERT ON dim_tenencia
FOR EACH ROW
WHEN (NEW.sk_tenencia IS NULL)
BEGIN
    :NEW.sk_tenencia := SEQ_DIM_TENENCIA.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_DIM_INDICADOR_HABITACAO
BEFORE INSERT ON dim_indicador_habitacao
FOR EACH ROW
WHEN (NEW.sk_indicador_habitacao IS NULL)
BEGIN
    :NEW.sk_indicador_habitacao := SEQ_DIM_INDICADOR_HABITACAO.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_DIM_FONTE
BEFORE INSERT ON dim_fonte
FOR EACH ROW
WHEN (NEW.sk_fonte IS NULL)
BEGIN
    :NEW.sk_fonte := SEQ_DIM_FONTE.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_FATO_SALARIO
BEFORE INSERT ON fato_salario
FOR EACH ROW
WHEN (NEW.sk_fato_salario IS NULL)
BEGIN
    :NEW.sk_fato_salario := SEQ_FATO_SALARIO.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_FATO_IMIGRACAO
BEFORE INSERT ON fato_imigracao
FOR EACH ROW
WHEN (NEW.sk_fato_imigracao IS NULL)
BEGIN
    :NEW.sk_fato_imigracao := SEQ_FATO_IMIGRACAO.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_FATO_HABITACAO
BEFORE INSERT ON fato_habitacao
FOR EACH ROW
WHEN (NEW.sk_fato_habitacao IS NULL)
BEGIN
    :NEW.sk_fato_habitacao := SEQ_FATO_HABITACAO.NEXTVAL;
END;
/
